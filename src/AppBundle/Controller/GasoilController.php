...
class GasoilController extends CommonController
{
    ...
    public function gasoilsAction(Request $request)
    {
        ...
        $gasoils = $em->getRepository('AppBundle:Gasoil')->getAllForCampagne($campagne);
        $gasoilsType = $em->getRepository('AppBundle:Gasoil')->getAllforCompanyGroupByType($this->company);

        $gasoilsAll = $em->getRepository('AppBundle:Gasoil')->getAllForCompany($this->company);
    ...
    public function gasoilEditAction($gasoil_id, Request $request)
    {
        ...
        if($gasoil_id == '0'){
            ...
            $gasoil = $em->getRepository('AppBundle:Gasoil')->findOneById($gasoil_id);
        }

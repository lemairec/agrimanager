...
class Bilan2Controller extends CommonController
{
    /**
    ...
    public function bilanDetailAction(Request $request)
    {
        ...
        foreach ($parcelles as $p) {
            ...
            $p->interventions = [];
            if($p->id != '0'){
                $p->interventions = $em->getRepository('AppBundle:Intervention')->getAllForParcelle($p);
            }
            $p->n = 0;
            $p->p = 0;
            $p->k = 0;
    ...
    /*public function bilanAction(Request $request)
    {
        ...
        $parcelles = $em->getRepository('AppBundle:Parcelle')->getAllForCampagneWithoutActive($campagne);
        ...
        foreach ($parcelles as $p) {
            ...
            $p->poid_norme = $em->getRepository('AppBundle:Livraison')->getSumForParcelle($p);
    ...
    public function bilanProduitsAction(Request $request)
    {
        ...
        $campagnes2 = $em->getRepository('AppBundle:Campagne')->getAllForCompany($this->getCurrentCampagne($request)->company);
        $cultures = [];

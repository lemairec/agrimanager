...
class CommonController extends Controller
{
    public function check_user($request){
        ...
        $user = $this->getUser();
        $this->company = $em->getRepository('AppBundle:Company')->findOrCreate($user);
    ...
    public function getCurrentCampagneId($request){
        ...
        if($campagne_id == ''){
            $em = $this->getDoctrine()->getManager();
            $campagne = $em->getRepository('AppBundle:Campagne')->findAll()[0]->id;
        }
    ...
    public function getCurrentCampagne($request){
        ...
        if(!property_exists($this, 'campagnes')){
            $this->campagnes = $em->getRepository('AppBundle:Campagne')->getAllforCompany($this->company);
        }

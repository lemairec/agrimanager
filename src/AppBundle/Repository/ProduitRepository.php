...
class ProduitRepository extends \Doctrine\ORM\EntityRepository
{
    function findOrCreate($completeName, $campagne){
        ...
        $ephy = $em->getRepository('AppBundle:EphyProduit')->findOneByCompleteName($completeName);
        if($ephy){
            $produit->type = "ppp";
    ...
    function findOrCreateCaj($name, $type, $unity,   $campagne){
        ...
        $produit->type = $type;
        $produit->unity = $unity;
        $ephy = $em->getRepository('AppBundle:EphyProduit')->findOneByCompleteName($name);
        /*if($ephy){
            $produit->type = "ppp";
        } else {
            $produit->type = "autre";
    ...
    function update($produit){
        ...
        $em->getRepository('AppBundle:ProduitCampagne')->update($produit);
        $ps = $em->getRepository('AppBundle:ProduitCampagne')->findByProduit($produit);
        $stock = 0;
        $price = 0;
        $nb = 0;
        foreach($ps as $p){
    ...
    function getAllName($campagne){
        ...
        $produits = $em->getRepository('AppBundle:EphyProduit')->findAll();
        foreach($produits as $p){
            $res[] = $p->completeName;
        }

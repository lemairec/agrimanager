...
class AchatRepository extends \Doctrine\ORM\EntityRepository
{
    function save($achat, $campagne){
        ...
        $produit = $em->getRepository('AppBundle:Produit')->findOrCreate($achat->name, $campagne);
    ...
    function addCaj($achat, $campagne){
        ...
        if(!empty($achat->externId)){
            $achats = $em->getRepository('AppBundle:Achat')->findByExternId($achat->externId);
            foreach($achats as $a){
                $em->remove($a);
                $em->flush();
            }
        ...
        $produit = $em->getRepository('AppBundle:Produit')->findOrCreateCaj($achat->name, $achat->type, $achat->unity, $campagne);
        $achat->produit = $produit;
        $achat->campagne = $campagne;
        $em->persist($achat);
        $em->flush();
    ...
    function addCajCsv($fileName){
        $em = $this->getEntityManager();
        $campagne = $em->getRepository('AppBundle:Campagne')->findOneById("f49d37d6-3951-11e7-92c4-80e65014bb7c");
